...
def getCoords(box):
    """물체 주의 bbox의 좌상단 우하단 좌표를 구한다.
    
    :param box: [box_x, box_y, box_w, box_h, class, confidence].
    :return: x1은 상자 좌측, x2는 우측, y1은 상단, y2는 하단 좌표를 의미.
    """
...
if __name__ == "__main__":
    # 이건 hparam이다. 모델의 표현력(test mAP를 기준으로 해서)에 따라 직접 설정.
    weights = [1.0, 0.1, 0.5]
    dets = [
    ...
    ens = GeneralEnsemble(dets, weights=weights)
    print(ens)
